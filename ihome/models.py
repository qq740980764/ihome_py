...
class User(BaseModel, db.Model):
    ...
    def to_dict(self):
        user_dict = {"id":self.id,
                "name":self.name,
                "mobile":self.mobile,
                "avatar_url": constants.QINIU_HTTP + self.avatar_url if self.avatar_url else "",
...
class House(BaseModel, db.Model):
    ...
    def to_full_dict(self):
            ...
            house_dict = {
                ...
                "user_avatar": constants.QINIU_HTTP + self.user.avatar_url if self.user.avatar_url else "",
                "title": self.title,
                "price": self.price,
            ...
            for image in self.images:
                img_urls.append(constants.QINIU_HTTP + image.url)
            house_dict["img_urls"] = img_urls

            # 房屋设施
            facilities_list = []
            # print("1234567",self.facilities)
            for facility in self.facilities:
                facilities_list.append(facility.id)
            house_dict["facilities"] = facilities_list
            print('房屋设置的只是123',self.facilities)

            # 评论信息
            comments = []
            orders = Order.query.filter(Order.id==self.id,Order.status=="COMPLETE",Order.comment !=None).order_by\
                (Order.update_time.desc()).limit(constants.HOT_ADVERT_NUM)
    ...
    def to_basic_dict(self):
        data_dict = {
            ...
            "img_url": constants.QINIU_HTTP + self.index_image_url if self.index_image_url else "",
            "room_count": self.room_count,
        ...
        # print(self.create_time,type(self.create_time))
        return data_dict
...
#     image = db.relationship("House_Image",backref="house")
#     orders = db.relationship("Order", backref="house")  # 房屋的订单
#     facilities = db.relationship("Facility", secondary=house_facility)  # 房屋的设施
#
#
